// ============================================================================
// Foreasy ESP32-S3
// Hardware: ESP32-S3 + shield relé 30A (Convencional) / saída START IN (Industrial)
//
...
// Industrial: dispara pulso no startPin (START IN), ativo HIGH — igual esp32_avail
void startPulse() {
...
// ================= AVAIL OUT (leitura com debounce) =================
// Lê o status da máquina. Ainda NÃO controla nada — só expõe em /status e /info.
void readAvailTick() {
...
void setup() {
  ...
  startWebServer();
  server.begin();
  Serial.printf("HTTP server iniciado. machineMode=%s\n",
                machineMode == MODE_INDUSTRIAL ? "INDUSTRIAL" : "CONVENCIONAL");
...
// ================= HTTP ROUTES =================
void startWebServer() {
  server.on("/",            handleRoot);            // redireciona para /config
...
<header><div class="logo">FOREASY</div><div class="sub">configuração do dispositivo</div></header>
<main>
  <div class="steps">
  ...
  <div class="card-wrap">
    ...
    <div class="step" id="step3">
      <div id="pinsCon">
        ...
        <input id="relayPin" type="number" min="0" max="48">
...
window.onload=()=>{
  fetch('/config-data').then(r=>r.json()).then(d=>{
    ...
    qs('relayPin').value=d.relayPin!=null?d.relayPin:2;
...
void handleSave() {
  ...
  if (server.hasArg("relayPin")) relayPin = constrain(server.arg("relayPin").toInt(), 0, 48);
  if (server.hasArg("startPin")) startPin = constrain(server.arg("startPin").toInt(), 0, 48);
  if (server.hasArg("availPin")) availPin = constrain(server.arg("availPin").toInt(), 0, 48);
